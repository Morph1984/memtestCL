...
cl_int softwaitForEvents(cl_uint num_events, const cl_event *event_list, cl_command_queue const *pcq, unsigned sleeplength, unsigned limit)
{
    ...
    if (status < 0)
        cerr << "Status was " << descriptionOfError(status) << endl;
    return status;
#else
    ...
    for (u32 i = 0; i < num_events; ++i)
    {
        ...
        {
            u32 current = getTimeMilliseconds();
            if ((current - start) > limit)
                return CL_INVALID_VALUE; // TODO: is this the best error?
            //cout << status << endl;
            SLEEPMS(sleeplength);
            err = clGetEventInfo(event_list[i], CL_EVENT_COMMAND_EXECUTION_STATUS, sizeof(cl_int), &status, NULL);
            if (err != CL_SUCCESS)
...
memtestState::memtestState(cl_context context, cl_device_id device) : ctx(context), dev(device), cq(clCreateCommandQueue(ctx, dev, 0, NULL)),
                                                                      memtest(ctx, dev, cq),
                                                                      ...
                                                                      allocated(false), hostTempMem(NULL), initTime(0)
{
    ...
    case CL_DEVICE_TYPE_GPU:
        nThreads = memtest.max_workgroup_size();
        break;
        break;
...
u32 memtestState::allocate(u32 mbToTest)
{
    ...
    try
    {
        ...
        devTempMem = clCreateBuffer(ctx, CL_MEM_READ_WRITE, sizeof(u32) * nBlocks, NULL, &err);
        if (err != CL_SUCCESS)
        {
            cerr << "Unable to allocate OpenCL memory: " << descriptionOfError(err) << endl;
            throw 2;
        ...
        hostTempMem = (u32 *)malloc(sizeof(u32) * nBlocks);
        if (hostTempMem == NULL)
        {
...
bool memtestState::gpuMemoryBandwidth(double &bandwidth, u32 mbToTest, u32 iters)
{
    if (!allocated || mbToTest > max_bandwidth_size())
        return false;
...
bool memtestState::writeConstant(const u32 constant) const
{
    if (!allocated)
        return false;
    cl_int status;
...
bool memtestState::verifyConstant(u32 &errorCount, const u32 constant) const
{
    if (!allocated)
        return false;
    cl_int status;
...
bool memtestState::gpuShortLCG0(u32 &errorCount, const u32 repeats) const
{
    ...
    if (status != CL_SUCCESS || softwaitForEvents(1, &event, &cq) != CL_SUCCESS)
        return false;
...
bool memtestState::gpuMovingInversionsPattern(u32 &errorCount, const u32 pattern) const
{
    if (!allocated)
        return false;
    u32 partialErrorCount;

    if (!writeConstant(pattern))
        return false;
    if (!verifyConstant(partialErrorCount, pattern))
        return false;
...
bool memtestState::gpuWalking8Bit(u32 &errorCount, const bool ones, const u32 shift) const
{
    if (!allocated)
        return false;
    cl_event event;
...
bool memtestState::gpuWalking32Bit(u32 &errorCount, const bool ones, const u32 shift) const
{
    if (!allocated)
        return false;
    cl_event event;
...
bool memtestState::gpuModuloX(u32 &errorCount, const u32 shift, const u32 pattern, const u32 modulus, const u32 overwriteIters) const
{
    if (!allocated)
        return false;
    cl_event event;
    cl_int status;
    u32 realShift = shift % modulus;
    errorCount = 0;
    u32 currentPattern = pattern;

    for (s32 i = 0; i < 2; ++i, currentPattern = ~currentPattern)
    {
...
cl_int memtestFunctions::setKernelArgs(cl_kernel &kernel, const s32 n_args, const size_t *sizes, const void **args) const
{
    char kername[256];
    clGetKernelInfo(kernel, CL_KERNEL_FUNCTION_NAME, 256, kername, NULL);
    for (s32 i = 0; i < n_args; ++i)
    {
...
u32 memtestFunctions::max_workgroup_size() const
{
    u32 maxsize = 0xFFFFFFFF;
    size_t kernelsize;
    for (s32 i = 0; i < n_kernels; ++i)
    {
...
cl_event memtestFunctions::writeConstant(const u32 nBlocks, const u32 nThreads, cl_mem base, u32 N, const u32 constant, cl_int &status) const
{
    cl_event event;
    const s32 n_args = 3;
    size_t sizes[] = {sizeof(cl_mem), sizeof(u32), sizeof(u32)};
    ...
    if (status != CL_SUCCESS)
    {
        cout << "Error " << descriptionOfError(status) << " queueing writeConstant kernel" << endl;
        return event;
    }
    return event;
}
cl_event memtestFunctions::writePairedConstants(const u32 nBlocks, const u32 nThreads, cl_mem base, u32 N, const u32 constant1, const u32 constant2, cl_int &status) const
{
    cl_event event;
    const s32 n_args = 4;
    size_t sizes[] = {sizeof(cl_mem), sizeof(u32), sizeof(u32), sizeof(u32)};
    ...
    if (status != CL_SUCCESS)
    {
        cout << "Error " << descriptionOfError(status) << " queueing writePairedConstants kernel" << endl;
        return event;
    }
    return event;
}
cl_event memtestFunctions::writeWalking32Bit(const u32 nBlocks, const u32 nThreads, cl_mem base, u32 N, const bool ones, const u32 shift, cl_int &status) const
{
    cl_event event;
    cl_int iones = (s32)ones;
    const s32 n_args = 4;
    size_t sizes[] = {sizeof(cl_mem), sizeof(u32), sizeof(cl_int), sizeof(u32)};
    ...
    if (status != CL_SUCCESS)
    {
        cout << "Error " << descriptionOfError(status) << " queueing writeWalking32Bit kernel" << endl;
        return event;
    }
    return event;
}
cl_event memtestFunctions::writeRandomBlocks(const u32 nBlocks, const u32 nThreads, cl_mem base, u32 N, const u32 seed, cl_int &status) const
{
    cl_event event;
    const s32 n_args = 4;
    size_t sizes[] = {sizeof(cl_mem), sizeof(u32), sizeof(u32), 4 * nThreads};
    ...
    if (status != CL_SUCCESS)
    {
        cout << "Error " << descriptionOfError(status) << " queueing writeRandomBlocks kernel" << endl;
        return event;
    }
    return event;
}
cl_event memtestFunctions::writePairedModulo(const u32 nBlocks, const u32 nThreads, cl_mem base, u32 N, const u32 shift, const u32 pattern1, const u32 pattern2, const u32 modulus, const u32 iters, cl_int &status) const
{
    cl_event event;
    const s32 n_args = 7;
    size_t sizes[] = {sizeof(cl_mem), sizeof(u32), sizeof(u32), sizeof(u32), sizeof(u32), sizeof(u32), sizeof(u32)};
    ...
    if (status != CL_SUCCESS)
    {
        cout << "Error " << descriptionOfError(status) << " queueing writePairedModulo kernel" << endl;
        return event;
    }
    return event;
}
cl_event memtestFunctions::shortLCG0(const u32 nBlocks, const u32 nThreads, cl_mem base, u32 N, const u32 repeats, const u32 period, cl_int &status) const
{
    cl_event event;
    const s32 n_args = 4;
    size_t sizes[] = {sizeof(cl_mem), sizeof(u32), sizeof(u32), sizeof(u32)};
    ...
    if (status != CL_SUCCESS)
    {
        cout << "Error " << descriptionOfError(status) << " queueing LCG0 kernel" << endl;
        return event;
    }
    return event;
}
cl_event memtestFunctions::shortLCG0Shmem(const u32 nBlocks, const u32 nThreads, cl_mem base, u32 N, const u32 repeats, const u32 period, cl_int &status) const
{
    cl_event event;
    const s32 n_args = 5;
    size_t sizes[] = {sizeof(cl_mem), sizeof(u32), sizeof(u32), sizeof(u32), sizeof(u32) * nThreads};
    ...
    if (status != CL_SUCCESS)
    {
        cout << "Error " << descriptionOfError(status) << " queueing LCG0Shmem kernel" << endl;
        return event;
    }
    return event;
}

u32 memtestFunctions::verifyConstant(const u32 nBlocks, const u32 nThreads, cl_mem base, u32 N, const u32 constant, cl_mem blockErrorCount, u32 *error_counts, cl_int &status) const
{
    const s32 n_args = 5;
    size_t sizes[] = {sizeof(cl_mem), sizeof(u32), sizeof(u32), sizeof(cl_mem), sizeof(u32) * nThreads};
    ...
    if (status != CL_SUCCESS)
    {
        cout << "Error " << descriptionOfError(status) << " queueing verifyConstant kernel" << endl;
        return (u32)-1;
    }
    status = clEnqueueReadBuffer(cq, blockErrorCount, CL_TRUE, 0, nBlocks * sizeof(u32), error_counts, 0, NULL, NULL);
    if (status != CL_SUCCESS)
    {
        cout << "Error " << descriptionOfError(status) << " queueing verifyConstant readback" << endl;
        return (u32)-1;
    }
...
u32 memtestFunctions::verifyPairedConstants(const u32 nBlocks, const u32 nThreads, cl_mem base, u32 N, const u32 constant1, const u32 constant2, cl_mem blockErrorCount, u32 *error_counts, cl_int &status) const
{
    const s32 n_args = 6;
    size_t sizes[] = {sizeof(cl_mem), sizeof(u32), sizeof(u32), sizeof(u32), sizeof(cl_mem), sizeof(u32) * nThreads};
    ...
    if (status != CL_SUCCESS)
    {
        cout << "Error " << descriptionOfError(status) << " queueing verifyPairedConstants kernel" << endl;
        return (u32)-1;
    }
    status = clEnqueueReadBuffer(cq, blockErrorCount, CL_TRUE, 0, nBlocks * sizeof(u32), error_counts, 0, NULL, NULL);
    if (status != CL_SUCCESS)
    {
        cout << "Error " << descriptionOfError(status) << " queueing verifyPairedConstants readback" << endl;
        return (u32)-1;
    }
...
u32 memtestFunctions::verifyWalking32Bit(const u32 nBlocks, const u32 nThreads, cl_mem base, u32 N, const bool ones, const u32 shift, cl_mem blockErrorCount, u32 *error_counts, cl_int &status) const
{
    cl_int iones = (s32)ones;
    const s32 n_args = 6;
    size_t sizes[] = {sizeof(cl_mem), sizeof(u32), sizeof(cl_int), sizeof(u32), sizeof(cl_mem), sizeof(u32) * nThreads};
    ...
    if (status != CL_SUCCESS)
    {
        cout << "Error " << descriptionOfError(status) << " queueing verifyWalking32Bit kernel" << endl;
        return (u32)-1;
    }
    status = clEnqueueReadBuffer(cq, blockErrorCount, CL_TRUE, 0, nBlocks * sizeof(u32), error_counts, 0, NULL, NULL);
    if (status != CL_SUCCESS)
    {
        cout << "Error " << descriptionOfError(status) << " queueing verifyWalking32Bit readback" << endl;
        return (u32)-1;
    }
...
u32 memtestFunctions::verifyRandomBlocks(const u32 nBlocks, const u32 nThreads, cl_mem base, u32 N, const u32 seed, cl_mem blockErrorCount, u32 *error_counts, cl_int &status) const
{
    const s32 n_args = 7;
    size_t sizes[] = {sizeof(cl_mem), sizeof(u32), sizeof(u32), sizeof(cl_mem), sizeof(u32) * nThreads, sizeof(u32) * nThreads, sizeof(u32) * nThreads};
    ...
    if (status != CL_SUCCESS)
    {
        cout << "Error " << descriptionOfError(status) << " queueing verifyRandomBlocks kernel" << endl;
        return (u32)-1;
    }
    status = clEnqueueReadBuffer(cq, blockErrorCount, CL_TRUE, 0, nBlocks * sizeof(u32), error_counts, 0, NULL, NULL);
    if (status != CL_SUCCESS)
    {
        cout << "Error " << descriptionOfError(status) << " queueing verifyRandomBlocks readback" << endl;
        return (u32)-1;
    }
...
u32 memtestFunctions::verifyPairedModulo(const u32 nBlocks, const u32 nThreads, cl_mem base, u32 N, const u32 shift, const u32 pattern1, const u32 modulus, cl_mem blockErrorCount, u32 *error_counts, cl_int &status) const
{
    const s32 n_args = 7;
    size_t sizes[] = {sizeof(cl_mem), sizeof(u32), sizeof(u32), sizeof(u32), sizeof(u32), sizeof(cl_mem), sizeof(u32) * nThreads};
    ...
    if (status != CL_SUCCESS)
    {
        cout << "Error " << descriptionOfError(status) << " queueing verifyModuloX kernel" << endl;
        return (u32)-1;
    }
    status = clEnqueueReadBuffer(cq, blockErrorCount, CL_TRUE, 0, nBlocks * sizeof(u32), error_counts, 0, NULL, NULL);
    if (status != CL_SUCCESS)
    {
        cout << "Error " << descriptionOfError(status) << " queueing verifyModuloX readback" << endl;
        return (u32)-1;
    }
...
u32 memtestMultiTester::allocate(u32 mbToTest)
{
    u32 totalmb = mbToTest;
    if (totalmb & 1)
        totalmb++;
    try
    {
        while (mbToTest > 0)
        {
            u32 amount = allocation_unit < mbToTest ? allocation_unit : mbToTest;
            //cout << "Allocating new tester of "<<amount<<" MiB \n";
            memtestState *tester = new memtestState(ctx, dev);
            if (!tester->allocate(amount))
                throw 1;
...
bool memtestMultiTester::gpuMemoryBandwidth(double &bandwidth, u32 mbToTest, u32 iters)
{
    if (!isAllocated())
        return false;
    if (mbToTest > max_bandwidth_size())
        return false;
    return testers.front()->gpuMemoryBandwidth(bandwidth, mbToTest, iters);
}
bool memtestMultiTester::gpuShortLCG0(u32 &errorCount, const u32 repeats) const
{
    u32 partialErrorCount;
    bool status;
    errorCount = 0;
...
bool memtestMultiTester::gpuShortLCG0Shmem(u32 &errorCount, const u32 repeats) const
{
    u32 partialErrorCount;
    bool status;
    errorCount = 0;
...
bool memtestMultiTester::gpuMovingInversionsOnesZeros(u32 &errorCount) const
{
    u32 partialErrorCount;
    bool status;
    errorCount = 0;
...
bool memtestMultiTester::gpuWalking8BitM86(u32 &errorCount, const u32 shift) const
{
    u32 partialErrorCount;
    bool status;
    errorCount = 0;
...
bool memtestMultiTester::gpuWalking8Bit(u32 &errorCount, const bool ones, const u32 shift) const
{
    u32 partialErrorCount;
    bool status;
    errorCount = 0;
...
bool memtestMultiTester::gpuMovingInversionsRandom(u32 &errorCount) const
{
    u32 partialErrorCount;
    bool status;
    errorCount = 0;
...
bool memtestMultiTester::gpuWalking32Bit(u32 &errorCount, const bool ones, const u32 shift) const
{
    u32 partialErrorCount;
    bool status;
    errorCount = 0;
...
bool memtestMultiTester::gpuRandomBlocks(u32 &errorCount, const u32 seed) const
{
    u32 partialErrorCount;
    bool status;
    errorCount = 0;
...
bool memtestMultiTester::gpuModuloX(u32 &errorCount, const u32 shift, const u32 pattern, const u32 modulus, const u32 overwriteIters) const
{
    u32 partialErrorCount;
    bool status;
    errorCount = 0;
...
u32 memtestMultiContextTester::allocate(u32 mbToTest)
{
    u32 totalmb = mbToTest;
    if (totalmb & 1)
        totalmb++;
